...
<template>
  <div v-if="store.isLoaded" class="h-screen flex flex-col overflow-hidden bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition-colors" :style="store.activeBoard?.background && store.currentView === 'board' ? { background: store.activeBoard.background.startsWith('data:') || store.activeBoard.background.startsWith('http') ? `url(${store.activeBoard.background}) center/cover no-repeat` : store.activeBoard.background } : {}">
    <header class="flex-shrink-0 bg-white/90 dark:bg-gray-800/90 backdrop-blur border-b border-gray-200 dark:border-gray-700 px-3 sm:px-6 py-2 sm:py-3 flex flex-wrap gap-y-2 sm:gap-y-3 justify-between items-center z-30">

      <div class="flex flex-wrap items-center gap-x-2 sm:gap-x-4 gap-y-2">
        <div class="flex items-center gap-2 bg-gray-100 dark:bg-gray-700 p-1 rounded-lg shrink-0">
        ...
        <h1 class="text-xl font-extrabold tracking-tight text-gray-950 dark:text-white ml-2 hidden xl:block shrink-0">Borodinskiy <span class="text-blue-600 dark:text-blue-400">Kanban</span></h1>

        <div v-if="store.currentView === 'board'" class="flex flex-wrap items-center gap-1 bg-gray-100 dark:bg-gray-700 rounded-md px-1 sm:pr-1 sm:ml-2 border border-transparent focus-within:border-gray-300 dark:focus-within:border-gray-500 transition-colors">
          ...
          <select v-model="store.settings.activeBoardId" class="bg-transparent border-none text-sm py-1.5 px-1 w-24 sm:w-48 md:w-64 min-w-0 focus:outline-none focus:ring-0 outline-none cursor-pointer text-gray-900 dark:text-white font-medium truncate flex-1">
      ...
      <div class="flex flex-wrap items-center gap-2 sm:gap-3">
        <div class="relative shrink-0">
          <button @click="isFilterOpen = !isFilterOpen" :class="['flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-lg text-sm font-medium transition-colors border', store.assigneeFilterIds.length > 0 ? 'bg-blue-50 border-blue-200 text-blue-700 dark:bg-blue-900/30' : 'bg-gray-100 border-transparent text-gray-600 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300']">
            <UsersIcon class="w-4 h-4" /> <span class="hidden sm:inline">Filter</span> <span v-if="store.assigneeFilterIds.length" class="ml-1 bg-blue-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-[10px]">{{store.assigneeFilterIds.length}}</span>
          </button>
          <div v-if="isFilterOpen" class="absolute top-full left-0 mt-2 w-64 max-w-[calc(100vw-1.5rem)] bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 z-50">
        ...
        <div class="relative flex items-center focus-within:ring-2 focus-within:ring-blue-500 transition-shadow rounded-lg z-30 shrink-0">
          <div class="pl-3 text-gray-400 absolute left-0 z-10"><MagnifyingGlassIcon class="w-4 h-4" /></div>
          <input v-model="searchQuery" type="text" placeholder="Search..." class="bg-gray-100 dark:bg-gray-700 border-none text-sm py-1.5 pl-9 pr-3 w-24 sm:w-40 md:w-48 lg:w-64 min-w-0 outline-none rounded-l-lg dark:text-white">
        ...
        <button @click="store.openNewTaskModal()" class="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-1.5 px-3 sm:px-4 rounded-md shadow-sm transition-colors shrink-0 flex items-center gap-1">
          <PlusIcon class="w-4 h-4" /><span class="hidden sm:inline">New Task</span>
        </button>
      </div>
    </header>
