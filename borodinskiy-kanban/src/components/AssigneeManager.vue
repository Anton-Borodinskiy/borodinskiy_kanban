...
<template>
  <div class="space-y-4">
    <div v-for="user in store.assignees" :key="user.id" class="flex items-center gap-4 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600">
      ...
      <div class="flex-1 grid grid-cols-2 gap-2">
        ...
        <input
          ...
          maxlength="3"
          class="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded px-2 py-1 text-sm uppercase"
        >
      ...
      <div class="flex flex-col gap-1">
        <button @click="user.avatar = null" v-if="user.avatar" class="text-xs text-blue-500 hover:underline">Reset Logo</button>
        <button @click="store.deleteAssignee(user.id)" class="text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 p-2 rounded">
    ...
    <button
      @click="store.addAssignee"
      class="w-full py-2 border-2 border-dashed border-gray-300 dark:border-gray-600 text-gray-500 hover:text-blue-500 hover:border-blue-500 transition-all rounded-lg text-sm font-medium"
    >
      + Add Employee
    </button>
  </div>
