  ...
  actions: {
    ...
    async deleteActiveBoard() {
      ...
      if(confirmed) {
         ...
         this.columns = this.columns.filter(c => c.boardId !== boardId); this.boards = this.boards.filter(b => b.id !== boardId)
    ...
    archiveTask(taskId) { const task = this.tasks.find(t => t.id === taskId); if (task) { task.isArchived = true; task.archivedAt = new Date().toISOString(); const col = this.columns.find(c => c.id === task.columnId); if (col) task.originalBoardId = col.boardId } },
    unarchiveTask(taskId) { const task = this.tasks.find(t => t.id === taskId); if (task && this.activeColumns.length > 0) { task.isArchived = false; task.columnId = this.activeColumns[0].id } },
    archiveAllInColumn(columnId) { const col = this.columns.find(c => c.id === columnId); this.tasks.filter(t => t.columnId === columnId && !t.isArchived).forEach(t => { t.isArchived = true; t.archivedAt = new Date().toISOString(); t.originalBoardId = col ? col.boardId : null }) },
    ...
    searchTasks(query, scope = 'current') {
      ...
      return tasksToSearch.filter(t => t.title?.toLowerCase().includes(q) || t.description?.toLowerCase().includes(q) || t.closingComment?.toLowerCase().includes(q))
    },
