  ...
  <div v-if="store.isLoaded" class="h-screen flex flex-col overflow-hidden text-gray-900 dark:text-gray-100 transition-colors" :style="store.activeBoard?.background && store.currentView === 'board' ? { background: store.activeBoard.background.startsWith('data:') || store.activeBoard.background.startsWith('http') ? `url(${store.activeBoard.background}) center/cover no-repeat` : store.activeBoard.background } : { backgroundColor: store.settings.theme === 'dark' || document?.documentElement?.classList.contains('dark') ? '#111827' : '#F9FAFB' }">
    <header class="flex-shrink-0 bg-white/90 dark:bg-gray-800/90 backdrop-blur border-b border-gray-200 dark:border-gray-700 px-6 py-3 flex justify-between items-center z-30">
      ...
      <div class="flex items-center gap-4">
        ...
        <div v-if="store.currentView === 'board'" class="flex items-center gap-1 bg-gray-100 dark:bg-gray-700 rounded-md pr-1 ml-2 border border-transparent focus-within:border-gray-300 dark:focus-within:border-gray-500 transition-colors">
          ...
          <button @click="renameActiveBoard" class="p-1 text-gray-500 hover:text-orange-600 dark:text-gray-400 rounded outline-none" title="Rename Board"><PencilIcon class="w-4 h-4" /></button>
          <button @click="createNewBoard" class="p-1 text-gray-500 hover:text-blue-600 dark:text-gray-400 rounded outline-none" title="Create New Board"><PlusIcon class="w-4 h-4" /></button>
          <button @click="store.duplicateBoard" class="p-1 text-gray-500 hover:text-green-600 dark:text-gray-400 rounded outline-none" title="Duplicate Board"><DocumentDuplicateIcon class="w-4 h-4" /></button>
          <button @click="store.isColumnsLocked = !store.isColumnsLocked" class="p-1 text-gray-500 hover:text-indigo-600 dark:text-gray-400 rounded outline-none border-l border-gray-300 dark:border-gray-600 pl-2 ml-1" :title="store.isColumnsLocked ? 'Unlock Columns' : 'Lock Columns'"><LockClosedIcon v-if="store.isColumnsLocked" class="w-4 h-4" /><LockOpenIcon v-else class="w-4 h-4 text-indigo-500" /></button>
          <button @click="store.deleteActiveBoard()" class="p-1 text-gray-500 hover:text-red-600 dark:text-gray-400 rounded outline-none" title="Delete Board"><TrashIcon class="w-4 h-4" /></button>
        </div>
      </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="relative flex items-center focus-within:ring-2 focus-within:ring-blue-500 transition-shadow rounded-lg z-30">
          ...
          <input v-model="searchQuery" type="text" placeholder="Search..." class="bg-gray-100 dark:bg-gray-700 border-none text-sm py-1.5 pl-9 pr-3 w-48 lg:w-64 outline-none rounded-l-lg dark:text-white">
        ...
        <button @click="store.toggleCompactMode" class="p-1.5 text-gray-500 hover:text-gray-900 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 rounded-md" :title="store.settings.isCompactMode ? 'Detailed View' : 'Compact View'">
           <Bars3Icon v-if="store.settings.isCompactMode" class="w-5 h-5" />
           <Squares2X2Icon v-else class="w-5 h-5" />
        </button>
        <button @click="store.toggleTheme" class="p-1.5 text-gray-500 hover:text-gray-900 bg-gray-100 dark:bg-gray-700 rounded-md"><MoonIcon v-if="store.settings.theme === 'light'" class="w-5 h-5" /><SunIcon v-else class="w-5 h-5" /></button>
        <button @click="store.openSettings" class="p-1.5 text-gray-500 hover:text-gray-900 bg-gray-100 dark:bg-gray-700 rounded-md"><Cog6ToothIcon class="w-5 h-5" /></button>
        <button @click="store.openNewTaskModal()" class="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-1.5 px-4 rounded-md shadow-sm">+ New Task</button>
      </div>
    </header>
